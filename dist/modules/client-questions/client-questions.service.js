...
exports.ClientQuestionsService = exports.ServiceError = void 0;
...
let ClientQuestionsService = class ClientQuestionsService {
    prisma;
    constructor() {
    ...
    async getClientProfileId(userId) {
        const profile = await this.prisma.client_profiles.findFirst({
            where: { user_id: userId },
        });
        if (!profile)
            throw new ServiceError("client_profile_not_found", 404);
        return profile.id;
    }
    async getAnswers(clientId, language) {
        ...
        return answers.map((a) => {
            ...
            return {
                ...
                answer: a.answer,
                answer_text: langQ?.choices[a.answer] ?? null,
                created_at: a.created_at,
                question: langQ?.question ?? null,
            };
        });
    }
    async createAnswers(clientId, answers) {
    ...
    async updateAnswers(clientId, answers) {
        const existing = await this.prisma.client_answers.findMany({
            where: { client_id: clientId, question_id: { in: answers.map((a) => a.question_id) } },
            select: { question_id: true },
        });
...
exports.ClientQuestionsService = ClientQuestionsService;
exports.ClientQuestionsService = ClientQuestionsService = __decorate([
    (0, tsyringe_1.injectable)(),
    __metadata("design:paramtypes", [])
], ClientQuestionsService);
//# sourceMappingURL=client-questions.service.js.map